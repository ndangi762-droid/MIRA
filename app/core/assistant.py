...
class MIRA:
    ...
    def _memory_command(self, message: str):
        ...
        if low in ("meri memory dikhao", "memory dikhao", "show my memories", "show memory", "what do you remember about me", "tumhe mere baare mein kya yaad hai"):
            items = self.memory.memories(limit=50)
            return "Boss, abhi long-term memory mein kuch saved nahi hai." if not items else "Boss, mujhe ye important baatein yaad hain:\n" + "\n".join(f"• [{x['category']}] {x['value']}" for x in items)
        ...
        for prefix in ("memory bhool jao ", "memory bhul jao ", "forget memory ", "forget "):
            if low.startswith(prefix):
                ...
                return f"Boss, {count} matching memory delete ho gayi{' hain' if count != 1 else ''}."
        return None

    def _needs_web_search(self, message: str) -> bool:
        text = message.lower().strip()
        if text.startswith(("search web ", "web search ", "internet par search ", "latest search ")): return True
        freshness = ("latest", "today", "aaj", "abhi", "current", "recent", "news", "update", "price", "stock price", "weather", "score", "result", "release", "launch", "version", "current ceo", "current president")
    ...
    def _action_reply(self, tool_name: str, result: dict) -> str:
        ...
        if tool_name == "list_files":
            files = result.get("files", []); return "Boss, MIRA workspace abhi empty hai." if not files else "Boss, workspace mein ye files hain:\n" + "\n".join(f"• {x}" for x in files[:50])
        if tool_name == "search_files":
            matches = result.get("matches", []); return f"Boss, '{result['query']}' ka koi match nahi mila." if not matches else "Boss, matches mile:\n" + "\n".join(f"• {x}" for x in matches[:50])
    ...
    def _build_prompt(self, message: str, history: list[dict]):
        ...
        style_block = "\n\n" + style_context + "\nUse these examples as STYLE guidance only." if style_context else ""
        smart_category = self.smart_memory.classify(message) if self.smart_memory.should_store(message) else None
        memory_notice = f"\n\nUseful memory category: {smart_category}." if smart_category else ""
    ...
    async def ask_document(self, filename: str, question: str, session_id: str = "boss") -> str:
        ...
        if not text.strip(): return "Boss, is document mein readable text nahi mila."
        system = SYSTEM_PROMPT + "\n\nDOCUMENT MODE: Answer from the uploaded document as the primary source. Do not invent facts. Reply naturally in Roman Hindi/Hinglish unless Boss asks otherwise." + f"\n\nDOCUMENT NAME: {document['file']}\nDOCUMENT TEXT:\n{text[:90000]}"
    ...
    async def stream(self, message: str, session_id: str = "boss") -> AsyncIterator[str]:
        ...
        if saved:
            system, _ = self._build_prompt(message, history); system += "\n\nIMPORTANT: Boss explicitly asked to save a memory. Confirm briefly."
            parts = []
            async for delta in self.llm.stream(system, history, message): parts.append(delta); yield delta
            response = "".join(parts)
        else:
            # Planning is used for explicit multi-step goals; normal chat stays fast.
            if self.planner.should_plan(message):
                plan = self.planner.create(message)
                response = self.planner.format(plan)
                yield response
            else:
                plan = await self.agent.plan(message, history, SYSTEM_PROMPT)
                if plan and plan.get("action") == "tool":
                    try: result = self.actions.execute(plan["tool"], **plan.get("args", {})); response = self._action_reply(plan["tool"], result)
