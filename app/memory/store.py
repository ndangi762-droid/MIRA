...
class MemoryStore:
    ...
    def __init__(self):
        ...
        with sqlite3.connect(MEMORY_DB) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_memories_category_updated ON memories(category, updated_at DESC)")
    ...
    def ensure_session(self, session_id: str, title: str = "New chat"):
        ...
        with sqlite3.connect(MEMORY_DB) as conn:
            conn.execute("INSERT OR IGNORE INTO sessions(id, title) VALUES (?, ?)", (session_id, title.strip() or "New chat"))
    ...
    def sessions(self, limit: int = 50):
        ...
        return [{"id": sid, "title": title, "created_at": created, "updated_at": updated} for sid, title, created, updated in rows]

    def rename_session(self, session_id: str, title: str):
        with sqlite3.connect(MEMORY_DB) as conn:
            conn.execute("UPDATE sessions SET title=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (title.strip() or "New chat", session_id))
    ...
    def memories(self, limit: int = 50, category: str | None = None):
        with sqlite3.connect(MEMORY_DB) as conn:
            if category:
                rows = conn.execute("SELECT key, value, category, updated_at FROM memories WHERE category=? ORDER BY updated_at DESC LIMIT ?", (category, limit)).fetchall()
            else:
                rows = conn.execute("SELECT key, value, category, updated_at FROM memories ORDER BY updated_at DESC LIMIT ?", (limit,)).fetchall()
        return [{"key": key, "value": value, "category": category_name, "updated_at": updated_at} for key, value, category_name, updated_at in rows]

    def find_memories(self, query: str, limit: int = 20):
        q = f"%{query.strip()}%"
        with sqlite3.connect(MEMORY_DB) as conn:
            rows = conn.execute(
        ...
        return [{"key": key, "value": value, "category": category, "updated_at": updated_at} for key, value, category, updated_at in rows]
    ...
    def forget_matching(self, query: str) -> int:
        q = f"%{query.strip()}%"
        with sqlite3.connect(MEMORY_DB) as conn:
            cur = conn.execute("DELETE FROM memories WHERE key LIKE ? OR value LIKE ?", (q, q))
            return cur.rowcount
