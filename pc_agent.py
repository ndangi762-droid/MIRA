...
APP_COMMANDS = {
    ...
    "explorer": ["explorer.exe"], "chrome": ["cmd", "/c", "start", "", "chrome"],
    "task_manager": ["taskmgr.exe"],
}
SITES = {"https://www.youtube.com/", "https://mail.google.com/", "https://www.google.com/", "https://github.com/"}


def open_app(name: str):
    command = APP_COMMANDS.get(name.lower().strip())
    if not command: return False, f"App '{name}' is not allowlisted."
    subprocess.Popen(command, shell=False)
    return True, f"{name.replace('_', ' ').title()} opened."


def open_url(url: str):
    if url not in SITES: return False, "Website is not allowlisted."
...
def search_web(query: str):
    query = query.strip()
    if not query: return False, "Search query is empty."
...
def type_text(value: str):
    if not value: return False, "Text is empty."
...
def hotkey(keys):
    ...
    if not keys or any(k not in allowed for k in keys): return False, "Hotkey is not allowlisted."
...
def click(x, y):
    ...
    if not (0 <= x < width and 0 <= y < height): return False, "Click coordinates are outside the screen."
    pyautogui.click(x, y)
    return True, f"Clicked at {x}, {y}."


def media_key(key):
    keys = {"volume_down": "volumedown", "volume_up": "volumeup", "play_pause": "playpause"}
    mapped = keys.get(str(key).lower())
    if not mapped: return False, "Media key is not allowlisted."
    pyautogui.press(mapped)
    return True, "Media control executed."


def open_and_type(target: str, value: str):
...
def execute(command: dict):
    ...
    if action in {"open_url", "open_website"}: return open_url(target)
    if action == "search_web": return search_web(target)
    if action == "type_text": return type_text(str(command.get("text", "")))
    if action == "open_and_type": return open_and_type(target, str(command.get("text", "")))
    if action == "hotkey": return hotkey(command.get("keys", []))
    if action == "click": return click(command.get("x", -1), command.get("y", -1))
    if action == "media_key": return media_key(command.get("key", ""))
    return False, "PC action is not allowlisted."


def poll(path: str):
    response = requests.get(f"{MIRA_SERVER}{path}", params={"token": TOKEN, "device": "windows"}, timeout=15)
...
def ack(path: str, command: dict, ok: bool, message: str):
    try:
        requests.post(f"{MIRA_SERVER}{path}", params={"token": TOKEN, "command_id": command.get("id", ""), "ok": str(ok).lower(), "message": message}, timeout=10)
