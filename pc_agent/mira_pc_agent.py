...
Run this on the user's Windows PC. It listens only on 127.0.0.1 and executes
an allow-listed set of harmless local actions. The cloud MIRA server does not
get arbitrary shell access through this agent.
"""
...
HOST = "127.0.0.1"
PORT = 8765
TOKEN = os.environ.get("MIRA_PC_TOKEN", "change-me")
...
def run_action(action: str, target: str = "") -> dict:
    ...
    if action == "open_website" and target in ALLOWED_URLS:
        webbrowser.open(ALLOWED_URLS[target])
        return {"ok": True, "message": f"Opened {target}."}

    if action == "open_app" and target in ALLOWED_APPS:
        subprocess.Popen(ALLOWED_APPS[target], shell=False)
        return {"ok": True, "message": f"Opened {target}."}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path != "/command":
            ...
            return
        if self.headers.get("X-MIRA-TOKEN") != TOKEN:
    ...
    def do_GET(self):
        if self.path == "/health":
            self._send(200, {"ok": True, "service": "mira-pc-agent"})
        else:
            self._send(404, {"ok": False, "message": "Not found."})

    def log_message(self, fmt, *args):
        print(f"[MIRA-PC] {fmt % args}")
